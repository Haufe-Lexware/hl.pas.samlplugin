...
class SAML2PluginTests(SAMLPluginTestsBase):
    ...
    def test_authenticate(self):
        ...
        creds['ssiauth'] = False
        got = plugin.authenticateCredentials(creds)
        self.assertTrue(got == None, 'authenticated in spite of invalid credentials, got %s' % got)
    ...
    def test_extract_credentials(self):
        ...
        self.assertEqual(resp.status, 302, 'Redirect not set, status is: %s' % resp.status)
        ...
        self.assertEqual(request.destination, 'http://nohost/auth/SSORedirect/metaAlias/idp')
        expected = 'http://nohost/'
        self.assertEqual(request.assertion_consumer_service_url, expected, 'unexpected assertion consumer service url, expected %s, got %s.' % (expected, request.assertion_consumer_service_url))
    ...
    def test_response_extraction(self):
        ...
        req.stdin = StringIO(urllib.parse.urlencode({'SAMLResponse' : encoded_response}))
        session = req.SESSION
        ...
        self.assertEqual(got, self.session_index, 'Expected session index %s, got %s' % (self.session_index, got))
    ...
    def test_passive_artifact_response(self):
        ...
        self.assertTrue(creds['login'] == 'thomas.schorr@haufe-lexware.com', 'unexpected credentials: %s' % creds)
        self.assertTrue(creds['ssiauth'], 'unexpected credentials: %s' % creds)
        self.assertTrue('SAMLart' not in req.form, 'artifact should have been deleted from request form')
    ...
    def test_active(self):
        ...
        self.assertTrue(result == True, 'challenge failed')
        got = session.get('_saml2_storedurl', '')
        self.assertTrue(got == referer, 'expected stored url %s in session, got %s' % (referer, got))
        # challenge should set the stored saml session id
        got = session.get('_saml2_sessid', '')
        self.assertTrue(got != '', 'Expected saml session id to be set in local session')
        self.assertTrue(session['_saml2_ssi_auth'] == False, 'challenge should reset the authentication flag in the session.')
    ...
    def test_challenge(self):
        ...
        self.assertTrue(response.headers['location'].endswith(urllib.parse.quote(test_url)))
    ...
    def test_slo(self):
        ...
        self.assertTrue(request.issuer.text == 'http://nohost/', 'wrong issuer, got: %s' % request.issuer.text)
        self.assertTrue(request.destination == 'http://nohost/auth/IDPSloRedirect/metaAlias/idp', 'wrong destination: %s' % request.destination)
        self.assertTrue(request.name_id.text == 'testtest', 'wrong NameID: %s' % request.name_id.text)
        self.assertTrue(not session.get('_saml2_ssi_auth', True), 'not logged out')
    ...
    def test_redirect_logout_request(self):
        ...
        self.assertTrue(response.status.status_code.value == 'urn:oasis:names:tc:SAML:2.0:status:Success', 'unexpected logout status: %s' % response.status.status_code.value)
        ...
        saml_response = urlvars.get('SAMLResponse')
        response = self._parse_logout_response(saml_response)
        self.assertTrue(response.status.status_code.value == 'urn:oasis:names:tc:SAML:2.0:status:RequestDenied', 'unexpected logout status: %s' % response.status.status_code.value)
    ...
    def test_checksession_passive_with_query(self):
        ...
        self.assertEqual(address, expected, 'unexpected stored url in session, expected %s, got %s.' % (expected, address))
        expected = dict([ item.split('=') for item in qs.split('&')])
        paramsOut = dict([ item.split('=') for item in querystring.split('&')])
        self.assertEqual(paramsOut, expected, 'unexpected querystring in stored url, expected: %s, got: %s' %(expected, paramsOut))
        ...
        for method in (plugin.active, plugin.passive):
            for ac in plugin.possible_authn_context_types:
                ...
                if ac == 'do not specify':
                    self.assertTrue(request.requested_authn_context is None, 'bogus request: %s' % request.to_string())
                    continue
                self.assertTrue(len(request.requested_authn_context.authn_context_class_ref) == 1, 'bogus request: %s' % request.requested_authn_context.to_string())
                got = request.requested_authn_context.authn_context_class_ref[0].text
                self.assertTrue(got == ac, 'unexpected authn context class - got %s, expected %s.' % (got, ac))
        ...
        for method in (plugin.active, plugin.passive, plugin.checksession):
            for ac in possible_authn_contexts:
                ...
                if ac == None:
                    ...
                    self.assertTrue(len(request.requested_authn_context.authn_context_class_ref) == 1, 'bogus request: %s' % request.requested_authn_context.to_string())
                    got = request.requested_authn_context.authn_context_class_ref[0].text
                    self.assertTrue(got.split('.')[-1] == ac, 'unexpected authn context class - got %s, expected %s.' % (got, ac))

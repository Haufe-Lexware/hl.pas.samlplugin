...
import logging
from .sigver import RSA_SHA1
from .sigver import REQ_ORDER
from .sigver import RESP_ORDER
from .sigver import RSASigner
from .sigver import sha1_digest
...
def parse_soap_enveloped_saml(text, body_class, header_class=None):
    ...
    for part in envelope:
        ...
        if part.tag == '{%s}Body' % NAMESPACE:
            for sub in part:
                try:
                    body = create_class_from_element_tree(body_class, sub)
                except Exception:
                    raise Exception(
            ...
            for sub in part:
                ...
                for klass in header_class:
                    ...
                    if sub.tag == "{%s}%s" % (klass.c_namespace, klass.c_tag):
                        header[sub.tag] = \
                            create_class_from_element_tree(klass, sub)
                        break
...
PACKING = {
    BINDING_HTTP_REDIRECT: http_redirect_message,
    BINDING_HTTP_POST: http_form_post_message,
}
